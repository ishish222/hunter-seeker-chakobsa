...
TracerRegisterReactions(self+0x232b,ST,0x0)
...
Start:
    TracerRegisterReactions(self+0x2339,LOAD_WININET,0x0)
    EnableBuiltin
    DumpMemory
    SecureAllSections
    TracerStartTrace
...
exception:
    Interrupt
    RunCmdHost(mkdir -p /mnt/1/output/logs/cleopatra_1_044)
    RunCmd(copy e:\logs\responder_log.txt \\10.0.2.4\qemu\logs\cleopatra_1_044)
    RunCmd(copy e:\server\log_*.txt \\10.0.2.4\qemu\logs\cleopatra_1_044)
    RunCmd(copy e:\logs\init_log.txt \\10.0.2.4\qemu\logs\cleopatra_1_044)
    RunCmd(copy e:\logs\last_log.txt \\10.0.2.4\qemu\logs\cleopatra_1_044)
    QemuQuit
